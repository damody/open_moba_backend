...
mod tests {
    ...
    fn test_basic_vision_calculation() {
        ...
        for (i, shadow) in result.shadows.iter().enumerate() {
            println!(
                "陰影 {}: 類型={:?}, 深度={:.2}",
                i, shadow.shadow_type, shadow.depth
            );
        }
    ...
    fn test_unobstructed_vision() {
        ...
        println!(
            "最大距離: {:.2}, 最小距離: {:.2}, 視野範圍: {:.2}",
            max_distance, min_distance, result.range
        );

        // 在無障礙物情況下，可見區域應該接近完整圓形
        assert!(
            max_distance <= result.range + 5.0,
            "最大距離不應該大幅超出視野範圍"
        );
        assert!(
            min_distance >= result.range - 5.0,
            "最小距離不應該大幅小於視野範圍"
        );
    }
    ...
    fn test_line_intersection() {
        ...
        if let Some(t) = intersection {
            ...
            assert!(
                intersection_point.y >= 0.0 && intersection_point.y <= 20.0,
                "Y座標應該在線段範圍內"
            );
        }
...
use super::{Bounds, ShadowCalculator};
use crate::comp::circular_vision::{ObstacleInfo, ObstacleProperties, ObstacleType};
use vek::Vec2;
