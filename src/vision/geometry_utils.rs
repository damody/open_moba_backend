...
impl GeometryUtils {
    ...
    pub fn ray_sector_intersection_improved(
        ...
        let intersection_angle = (intersection_point - center)
            .y
            .atan2((intersection_point - center).x);
        ...
        let second_intersection_angle = (second_intersection_point - center)
            .y
            .atan2((second_intersection_point - center).x);

        // 檢查兩個交點，返回最近的在扇形內的交點
        let first_in_sector = Self::angle_in_sector(intersection_angle, start_angle, end_angle);
        let second_in_sector =
            Self::angle_in_sector(second_intersection_angle, start_angle, end_angle);
