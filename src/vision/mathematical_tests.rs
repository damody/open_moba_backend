...
mod tests {
    ...
    fn test_circular_shadow_mathematics() {
        ...
        assert_eq!(
            shadow.shadow_type,
            crate::comp::circular_vision::ShadowType::Sector
        );

        if let crate::comp::circular_vision::ShadowGeometry::Sector {
            center,
            start_angle,
            end_angle,
            radius,
        } = &shadow.geometry
        {
            ...
            assert!(
                (start_angle - expected_start).abs() < 0.01,
                "起始角度不正確"
            );
    ...
    fn test_ray_sector_intersection_mathematics() {
        ...
        println!(
            "實際距離: {}, 期望近點: {}, 期望遠點: {}",
            distance, expected_first, expected_second
        );

        // 由於測試的扇形覆蓋了整個相交區域，應該返回最近的交點
        assert!(
            (distance - expected_first).abs() < 1.0 || (distance - expected_second).abs() < 1.0,
            "相交距離不正確: {} (期望 {} 或 {})",
            distance,
            expected_first,
            expected_second
        ...
        println!(
            "邊界測試 - 射線方向: {:?}, 角度: {:.3}度",
            ray_direction_boundary,
            boundary_angle.to_degrees()
        );
    ...
    fn test_angle_normalization() {
        ...
        // 測試正常角度範圍
        assert!(
            ShadowCalculator::angle_in_sector(0.5, 0.0, 1.0),
            "角度在範圍內"
        );
        assert!(
            !ShadowCalculator::angle_in_sector(1.5, 0.0, 1.0),
            "角度不在範圍內"
        );
        ...
        assert!(
            ShadowCalculator::angle_in_sector(0.0, start_angle, end_angle),
            "0度應該在跨越0度的扇形內"
        );
        assert!(
            ShadowCalculator::angle_in_sector(2.0 * PI - 0.1, start_angle, end_angle),
            "359.9度應該在扇形內"
        );
        assert!(
            ShadowCalculator::angle_in_sector(0.1, start_angle, end_angle),
            "0.1度應該在扇形內"
        );
        assert!(
            !ShadowCalculator::angle_in_sector(PI, start_angle, end_angle),
            "180度不應該在扇形內"
        );
    }
    ...
    fn test_sector_overlap_detection() {
        ...
        // 測試明顯重疊
        assert!(
            ShadowCalculator::sectors_overlap_or_adjacent(0.0, PI / 2.0, PI / 4.0, 3.0 * PI / 4.0),
            "重疊的扇形應該被檢測出來"
        );

        // 測試不重疊但相鄰
        assert!(
            ShadowCalculator::sectors_overlap_or_adjacent(0.0, PI / 4.0, PI / 4.0, PI / 2.0),
            "相鄰的扇形應該被檢測出來"
        );

        // 測試完全分離
        assert!(
            !ShadowCalculator::sectors_overlap_or_adjacent(0.0, PI / 6.0, PI / 2.0, 2.0 * PI / 3.0),
            "分離的扇形不應該被檢測為重疊"
        );

        // 測試跨越0度的情況
        assert!(
            ShadowCalculator::sectors_overlap_or_adjacent(11.0 * PI / 6.0, PI / 6.0, 0.0, PI / 4.0),
            "跨越0度的重疊扇形應該被檢測出來"
        );
    }
    ...
    fn test_numerical_stability() {
        ...
        assert!(
            !large_result.visible_area.is_empty(),
            "大視野範圍下可見區域不應該為空"
        );
    }
}
