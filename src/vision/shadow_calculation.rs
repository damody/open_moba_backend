use crate::comp::circular_vision::{
    ObstacleInfo, ObstacleType, ShadowArea, ShadowGeometry, ShadowType,
};
use vek::Vec2;
        ...
        for next in sectors.into_iter().skip(1) {
            ...
            ) = (&current.geometry, &next.geometry)
            {
                if curr_center.distance(*next_center) < 1.0
                    ...
                    && Self::sectors_overlap_or_adjacent(
                        *curr_start,
                        *curr_end,
                        *next_start,
                        *next_end,
        ...
        for i in 0..precision {
            ...
            for shadow in shadows {
                if let Some(intersection) =
                    Self::ray_shadow_intersection(observer_pos, direction, shadow)
                {
