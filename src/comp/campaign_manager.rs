...
impl CampaignManager {
    pub fn init_campaign_data(ecs: &mut World, campaign_data: &CampaignData) {
        log::info!(
            "Initializing campaign data for: {}",
        ...
        for stage_data in &campaign_data.mission.stages {
            let stage =
                Stage::from_campaign_data(stage_data, campaign_data.mission.campaign.id.clone());
    ...
    fn create_hero_entity(
        ...
        log::info!(
            "Created hero entity '{}' with full combat components",
            hero_data.id
        );
    ...
    fn create_hero_abilities(
        _ecs: &mut World,
        _hero_entity: specs::Entity,
        ability_ids: &[String],
    ...
    fn create_training_enemies(ecs: &mut World, campaign_data: &CampaignData) {
        ...
        for (i, (x, y)) in enemy_positions.iter().enumerate() {
                ...
                .get(i % campaign_data.entity.enemies.len())
            {
                ...
                log::info!(
                    "Created training enemy unit '{}' at position ({}, {})",
                    enemy_data.id,
                    x,
                    y
                );
    ...
    fn create_training_creeps(ecs: &mut World, campaign_data: &CampaignData) {
        ...
        for (i, (x, y)) in creep_positions.iter().enumerate() {
            if let Some(creep_data) = campaign_data
                .entity
                .creeps
                .get(i % campaign_data.entity.creeps.len())
            {
                ...
                log::info!(
                    "Created training creep unit '{}' at position ({}, {})",
                    creep_data.id,
                    x,
                    y
                );
