...
impl Unit {
    ...
    pub fn from_creep_data(creep_data: &crate::ue4::import_campaign::CreepJD) -> Self {
        ...
        let s = active_creep_stats(cid).unwrap_or_else(|| {
            panic!(
                "creep '{}' has no stats in generated templates",
                creep_data.id
            )
        });
        ...
        let attack_range = if s.attack_range.raw() > 0 {
            s.attack_range
        } else {
        ...
        Unit {
            ...
            name: active_creep_display(cid).to_string(),
            unit_type,
