...
impl ShadowCaster {
    ...
    fn calculate_rectangular_shadow(
        ...
        let projected_vertices =
            self.project_edges_to_circle(observer_pos, vision_range, &visible_edges);
    ...
    /// 找到矩形的可見邊緣
    fn find_visible_edges(&self, observer_pos: Vec2<f32>, corners: &[Vec2<f32>]) -> Vec<Vec2<f32>> {
    ...
    fn calculate_visible_area(
        ...
        for i in 0..self.ray_precision {
            ...
            for shadow in shadows {
                if let Some(intersection_distance) =
                    self.ray_intersects_shadow(observer_pos, direction, shadow)
                {
