...
impl TerrainHeightMap {
    ...
    fn apply_circle_region(&mut self, region: &TerrainRegion, terrain_type: TerrainType) {
        ...
        let grid_min_x =
            (((region.center_x - radius) / self.grid_size).floor() as i32).max(0) as usize;
        let grid_max_x = (((region.center_x + radius) / self.grid_size).ceil() as i32)
            .min(self.width as i32) as usize;
        let grid_min_y =
            (((region.center_y - radius) / self.grid_size).floor() as i32).max(0) as usize;
        let grid_max_y = (((region.center_y + radius) / self.grid_size).ceil() as i32)
        ...
        for y in grid_min_y..grid_max_y {
            for x in grid_min_x..grid_max_x {
                ...
                if center.distance(pos) <= radius {
                    ...
                    self.heights[y][x] = region.terrain_height * distance_factor
                        + self.heights[y][x] * (1.0 - distance_factor);
                    self.terrain_types[y][x] = terrain_type;
    ...
    /// 檢查兩點之間的視線是否被地形遮擋
    pub fn is_line_of_sight_blocked(
        &self,
        from: Vec2<f32>,
        to: Vec2<f32>,
        observer_height: f32,
    ...
    fn deduplicate_positions(&self, positions: Vec<Vec2<f32>>) -> Vec<Vec2<f32>> {
        ...
        for pos in positions {
            ...
            if !unique_positions
                .iter()
                .any(|&existing: &Vec2<f32>| existing.distance(grid_pos) < self.grid_size * 0.1)
            {
    ...
    pub fn debug_heightmap(&self, area: (Vec2<f32>, Vec2<f32>)) -> String {
        ...
        let max_grid = self
            .world_to_grid(max_pos)
            .unwrap_or((self.width - 1, self.height - 1));
